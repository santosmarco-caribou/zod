/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/ban-types, unicorn/prefer-export-from */
...
export namespace utils {
  ...
  export type Simplify<T> = { 0: { [K in keyof T]: Simplify<T[K]> }; 1: T }[Or<[Equals<T, unknown>, Extends<T, _internals.BuiltIn | AnyZ>]>]
  ...
  export type EnforceOptional<T> = { [K in keyof T as RequiredFilter<T, K>]: T[K] } & {
    [K in keyof T as OptionalFilter<T, K>]?: Exclude<T[K], undefined>
  }
  ...
  export type RequireAtLeastOne<T, K extends keyof T = keyof T> = {
    [K_ in K]-?: Required<Pick<T, K_>> & Partial<Pick<T, Exclude<K, K_>>>
  }[K] &
    utils.StrictOmit<T, K>
  ...
  export const sortKeys = <T extends object>(object: T) =>
    fromEntries(
      keys(object)
        .sort()
  ...
  export const mergeDeep = <A, B>(a: A, b: B) => _mergeWith(_cloneDeep(a), _cloneDeep(b), (objectValue, sourceValue) => (Array.isArray(objectValue) ? [...objectValue, ...sourceValue] : undefined))
